...
namespace fv
{
    defineTypeNameAndDebug(actuationDiskRingsV2_Source, 0);
...
void Foam::fv::actuationDiskRingsV2_Source::checkData() const
{

    if (magSqr(diskArea_) <= VSMALL)
    {
        FatalErrorIn("Foam::fv::actuationDiskRingsV2_Source::checkData()")
...
Foam::fv::actuationDiskRingsV2_Source::actuationDiskRingsV2_Source
(
    const word& name,
...
:
    ...
    pitch_(readScalar(coeffs_.lookup("pitch"))),
    ...
    profiles_(coeffs_.subDict("profiles"))
{
    ...
    Info<< "    - creating actuationDiskRingsV2_Source: "
        << this->name() << endl;
	...
	(*outTurbines) << "0-Turbine, 1-time [s], 2-Uinf(fixed) [m/s], 3-Cp(fixed), 4-Ct(fixed), 5-Power(Uinf,Cp) [W], 6-Thrust(Uinf,Ct) [N] , 7-Center Ud [m/s], 8-Average Ud [m/s], 9-Uinf(disc cells) [m/s], 10-Power(disc cells) [W], 11-Thrust(disc cells) [N], 12-Torque(disc cells) [Nm], 13- pitch [deg]" << std::endl; //Head file
	}
...
void Foam::fv::actuationDiskRingsV2_Source::addSup
(
    fvMatrix<vector>& eqn,
    const label fieldI
)
{
    ...
    if (V() > VSMALL)
    {
        addactuationDiskRingsV2_AxialInertialResistance
        (
            Usource,
            cells_,
            cellsV,
...
bool Foam::fv::actuationDiskRingsV2_Source::read(const dictionary& dict)
{
