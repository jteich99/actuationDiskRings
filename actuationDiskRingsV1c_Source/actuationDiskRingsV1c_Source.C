...
namespace fv
{
    defineTypeNameAndDebug(actuationDiskRingsV1c_Source, 0);
    addToRunTimeSelectionTable
    (
        option,
        actuationDiskRingsV1c_Source,
        dictionary
    );
}
}
...
void Foam::fv::actuationDiskRingsV1c_Source::checkData() const
{

    if (magSqr(diskArea_) <= VSMALL)
    {
        FatalErrorIn("Foam::fv::actuationDiskRingsV1c_Source::checkData()")
...
Foam::fv::actuationDiskRingsV1c_Source::actuationDiskRingsV1c_Source
(
    const word& name,
...
{   
    ...
    Info<< "    - creating actuationDiskRingsV1c_Source: "
        << this->name() << endl;
	...
	if (Pstream::myProcNo() == 0) //In Master node
	    {
	(*outTurbines) << "---ADRingsV1c---" << std::endl; //Head file
	...
	if (Pstream::myProcNo() == 0) //In Master node
	    {
	(*outRings) << "---ADRingsV1c---" << std::endl; //Head file
	}
...
void Foam::fv::actuationDiskRingsV1c_Source::addSup
(
    fvMatrix<vector>& eqn,
    const label fieldI
)
{
    ...
    if (V() > VSMALL)
    {
        addactuationDiskRingsV1c_AxialInertialResistance
        (
            Usource,
            cells_,
            cellsV,
...
bool Foam::fv::actuationDiskRingsV1c_Source::read(const dictionary& dict)
{
