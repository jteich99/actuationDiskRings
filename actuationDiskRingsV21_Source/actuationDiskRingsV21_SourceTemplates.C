    ...
    for (int ring = 0; ring <= (numberRings_); ring = ring + 1)
    {
        ...
        for (int nodeIterator = 1; nodeIterator <= ringNodesList_[ring]; nodeIterator += 1)
        {
            ...
            forAll(cellsDisc, c)
            {
                ...
                force[cellsDisc[c]] += (((Vcells[cellsDisc[c]] * weightCells[cellsDisc[c]]) / V_point_F) * F_n_Bi) * uniDiskDir * -1 * density_ / pow(cellSize_ * 0.5, 2); 
                ...
                force[cellsDisc[c]] += (((Vcells[cellsDisc[c]] * weightCells[cellsDisc[c]]) / V_point_F) * F_tita_Bi) * F_tita_dir * -1 * density_ / pow(cellSize_ * 0.5, 2); 
